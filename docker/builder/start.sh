...
PIPFILE_DEPENDENCY_DEFINITION=`cat Pipfile | grep geo-knowledge-hub`

# extracting the version with perl-based code from oHo's answer: 
...
git clone --branch ${PIPFILE_DEPENDENCY_VERSION} ${GEO_KNOWLEDGE_HUB_ORGANIZATION_URL}/geo-knowledge-hub

# extract the dependencies from the `geo-knowledge-hub`
# note: `@geo` is the scope defined to the `geo-knowledge-hub` packages
JAVASCRIPT_DEPENDENCIES_VERSION=`cat geo-knowledge-hub/geo_knowledge_hub/modules/theme/webpack.py | grep -e @geo-knowledge-hub`
...
rm -rf geo-knowledge-hub ${FIRST_PACKAGE//@geo-knowledge-hub\/''}
